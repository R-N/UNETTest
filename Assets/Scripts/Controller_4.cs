...
public class Controller_4 : NetworkBehaviour {
	...
	const float DummySyncInterval = 0.08f;
	...
	public class PlyMoveOutput{
		...
		public void UnpackOutputToThis(PackedOutput pack){
			...
			string[] newGrav = pack.gravIds.Split('#');
			string[] newImp = pack.impulseIds.Split('#');
			for (int a = 0; a < updates; a++){
				gravIds[a] = byte.Parse(newImp[a]);
	...
	public struct PackedOutput{
		public Vector3 accResDeltaPos;
		public string gravIds;
		public string impulseIds;
		...
		public static PackedOutput PackOutput(PlyMoveOutput parent){
			...
			for (int a = 0; a < PlyMoveInput.updates; a++) {
				...
				ret.gravIds += parent.gravIds [a].ToString ();
				ret.impulseIds += parent.impulseIds [a].ToString ();
			}
			return ret;
		}
	}
	...
	//links
	public static Controller_4 player = null;
	...
	Vector3 tempTMove = Vector3.zero;
	...
	ServerOutput myServerOutput = new ServerOutput();
	...
	void Start(){
		...
		//serverOutputs.Add (0, new ServerOutput ());
	...
	IEnumerator CheckSync(float interval){
		while (true) {
			if (pendingState.Count > 0) {
				CmdCheckSync (pendingState [pendingState.Count - 1].pos, pendingState [pendingState.Count - 1].state);
			} else {
				CmdCheckSync (transform.position, state);
			}
	...
	void ActualFixedUpdate(){
		...
		if (isLocalPlayer || isServer) {
			...
			int curCount = -1;
			while (savedDt >= Time.fixedDeltaTime && !waitingForInput) {
				TryGetInput ();
			...
			if (savedDt < Time.fixedDeltaTime){
				...
				while(inputs.Count > b + 1){
					if (!serverOutputs.ContainsKey(inputs[b].timeStamp))
						serverOutputs.Add(inputs[b].timeStamp, new ServerOutput(true));
					inputs.RemoveAt(b);
				}
			}
				TryGetInput ();
	...
	void LerpDummyState(float dtPerState){
		if (pendingState.Count > 0){
			float dt = dtPerState / Mathf.Clamp(pendingState.Count, 1, pendingState.Count);
			while (accUpdateDt >= dt && pendingState.Count > 0) {
				curState = pendingState [0];
				pendingState.RemoveAt (0);
				accUpdateDt -= dt;
				dt = dtPerState / Mathf.Clamp(pendingState.Count, 1, pendingState.Count);
			}
			state = curState.state;
			grounded = curState.grounded;
			if (pendingState.Count > 0) {
	...
	[Command]
	void CmdCheckSync(Vector3 newClientPos, byte clientState){
		float dist = 0;
		if (pendingState.Count > 0){
	...
	void ForceSync(){
		...
		UpdateGround ();
		if (pendingState.Count > 0) {
			RpcForceSync (pendingState[pendingState.Count - 1].pos, pendingState[pendingState.Count - 1].rot, gravity, gravAccMove, timeSinceUngrounded, pendingState[pendingState.Count - 1].grounded, pendingState[pendingState.Count - 1].state);
		} else {
			RpcForceSync (transform.position, transform.rotation, gravity, gravAccMove, timeSinceUngrounded, grounded, state);
		}
	...
	void LocalUpdate(float dt){
		...
		if (AddGravitySelf (tempGrav))
			myPlyOutput.gravIds [myInput.updatesDone] = lastGravityId;
		else
			myPlyOutput.gravIds [myInput.updatesDone] = 0;
		if (AddImpulseSelf (tempImpulse, tempImpulseDeccel, true))
			myPlyOutput.impulseIds [myInput.updatesDone] = lastClientImpulseId;
		else
			myPlyOutput.impulseIds [myInput.updatesDone] = 0;

		myInput.UpdateDone ();
		if (myInput.updatesDone >= 3){
			if (!isServer){
				CmdSendOutput (myInput.timeStamp, PackedOutput.PackOutput (myPlyOutput));
			}
			TryGetInput ();
			myPlyOutput.Clean ();
		}
	...
	void ServerUpdate(float dt, int curCount, int doCount){
		...
		if (!serverOutputs.ContainsKey (myInput.timeStamp)) {
			serverOutputs.Add (myInput.timeStamp, new ServerOutput ());
		}
		...
		if (AddImpulseSelf (tempImpulse, tempImpulseDeccel)) {
			serverOutputs [myInput.timeStamp].impulseIds [myInput.updatesDone] = lastServerImpulseId;
		} else {
			serverOutputs [myInput.timeStamp].impulseIds [myInput.updatesDone] = 0;
		}
	...
	[Command]
	void CmdSendOutput(byte timeStamp, PackedOutput pack){
	...
	IEnumerator WaitToCheckIds(byte timeStamp, PlyMoveOutput output){
		for (int b = 0; b < 1200; b++){
			if (serverOutputs.ContainsKey(timeStamp) && serverOutputs[timeStamp].done){
				...
				RpcSendPlyCorrection (curOutput.accResDeltaPos - output.accResDeltaPos);
				...
				for (int a = 0; a < PlyMoveInput.updates; a++) {
					...
					if (output.gravIds[a] == 0) {
						if (curGrav != null) {
							RpcSendAddGrav (tempGrav, timeSinceUngrounded);
						} 
					} else {
						if (curGrav == null) {
							RpcSendGravDecline (output.gravIds[a]);
						} else {
							RpcSendGravCorrection (output.gravIds[a], curGrav.velocity, curGrav.applyTimeSinceUngrounded);
						}
					}
					...
					if (plyImpId == 0) {
						if (curImpId != 0) {
							...
							RpcSendImpulse (curImp.impulseStart, curImp.impulseDeccel, curImpId);
						}
					} else {
						if (curImpId == 0) {
							RpcSendImpulseDecline (output.impulseIds[a]);
						} else {
							curImp = serverImpulse [curImpId];
							RpcSendImpulseCorrection (curImp.impulseStart, curImp.impulseDeccel, plyImpId, curImpId);
		...
		for (int a = impIds.Count - 1; a >= 0; a -= 1) {
			...
			if (!imps [impIds [a]].done) {
				...
				if (curImp.done && isServer) {
					...
					if (!isLocalPlayer) {
						if (player) {
							RpcSendImpulseDonePlayer (id, curImp.impulseMoveDone);
						} else {
							RpcSendImpulseDoneServer (id, curImp.impulseMoveDone);
						}
					}
					impIds.RemoveAt (a);
					imps.Remove (id);
	...
	bool AddTempGravity(Vector3 grav){
	...
	bool AddTempImpulse(Vector3 imp, float deccel){
		if (imp == Vector3.zero) {
			return false;
		}
	...
	public bool AddImpulseSelf(Vector3 addImp, float addDeccel, bool fromPlayer = false){
		...
		if (!isServer || fromPlayer) {
			...
			while (serverImpulse.ContainsKey (lastClientImpulseId)) {
				if (lastClientImpulseId == byte.MaxValue)
					lastClientImpulseId = 1;
				else
					lastClientImpulseId++;
	...
	public bool AddGravityExt(Vector3 addGrav){
		...
		gravAccMove += delta;
		if (!isLocalPlayer)
			RpcSendAddGrav (addGrav, timeSinceUngrounded);
		return true;
	}

	public bool AddGravitySelf(Vector3 addGrav){
		if (addGrav == Vector3.zero) {
			return false;
		}
		Vector3 curPos = transform.position;
	...
	void PlayerUpdate(PlyMoveInput input, float dt){
		...
		if (grounded) {
			if (GetButtonDown(input.bts[JumpBt])) {
				...
				tempImpulse += input.dir.normalized * 4 * mvSpd;
	...
	[Command]
	void CmdSendInput(PackedInput pack){
		if (!inited) {
			inited = true;
			InitAccDt ();
			pendingState.Clear ();
		...
		if (serverOutputs.ContainsKey (parsed.timeStamp)) 
			...
			serverOutputs.Add (parsed.timeStamp, new ServerOutput ());
		inputs.Add (parsed);
	...
	void CmdSameInput(byte ts){
		...
		if (serverOutputs.ContainsKey (ts)) 
			...
			serverOutputs.Add (ts, new ServerOutput ());
		inputs.Add (PlyMoveInput.CopyFromInput (lastSentRecInput));
	...
	[ClientRpc]
	void RpcSendGravCorrection(byte id, Vector3 grav, float time){
		if (isLocalPlayer) {
	...
	[ClientRpc]
	void RpcSendGravDecline(byte id){
		if (isLocalPlayer) {
			if (pendingGrav.ContainsKey (id)) {
				Vector3 correction = -pendingGrav [id].accMove;
	...
	void CmdStomp(Vector3 pos){
		...
		Controller_4 con = null;
		foreach (Collider a in cols) {
			con = a.GetComponent<Controller_4> ();
	...
	[ClientRpc]
	void RpcSendImpulseCorrection(Vector3 imp, float deccel, byte id, byte serverId){
		if (isLocalPlayer) {
	...
	[ClientRpc]
	void RpcSendImpulseDecline(byte id){
		if (isLocalPlayer) {
			if (playerImpulse.ContainsKey (id)) {
	...
	[ClientRpc]
	void RpcSendImpulseDonePlayer(byte id, Vector3 accMove){
		if (playerImpulse.ContainsKey (id)) {
	...
	[ClientRpc]
	void RpcSendImpulseDoneServer(byte id, Vector3 accMove){
	...
	[ClientRpc]
	void RpcForceSync(Vector3 pos, Quaternion rot, Vector3 grav, Vector3 gravAcc, float timeUnground, bool newGrounded, byte newState){
		if (isLocalPlayer) {
	...
	[ClientRpc]
	void RpcForceImpulse(Vector3 impulseLeft, float deccel, Vector3 impulseDone, float impTime, byte id){
		if (isLocalPlayer) {
			if (serverImpulse.ContainsKey (id)) {
	...
	[ClientRpc]
	void RpcSendImpulse(Vector3 impulse, float deccel, byte id){
		if (isLocalPlayer) {
			serverImpulseIds.Add (id);
			serverImpulseIds.Sort ();
